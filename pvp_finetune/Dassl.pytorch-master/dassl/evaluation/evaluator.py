...
@EVALUATOR_REGISTRY.register()
class MLClassificationDual(EvaluatorBase):
    """Evaluator for classification."""
...
class MLClassification(EvaluatorBase):
    ...
    def process(self, text_global, image_global, gt, text_local, image_local=None):
        # gt (torch.LongTensor): ground truth [batch]
        
        self._y_true.append(gt.data)
        self._y_text_global.append(text_global.data)
        self._y_image_global.append(image_global.data)
        if text_local is not None:
            self._y_text_local.append(text_local.data)
        if image_local is not None:
            self._y_image_local.append(image_local.data)
    ...
    def evaluate(self):
        ...
        targets = torch.cat(self._y_true, dim=0).cpu().numpy()
        preds_text = torch.cat(self._y_text_global, dim=0)
        preds_image = torch.cat(self._y_image_global, dim=0)

        if len(self._y_text_local) > 0:
            preds_aux_text = torch.cat(self._y_text_local, dim=0)
        if len(self._y_image_local) > 0:
            preds_aux_image = torch.cat(self._y_image_local, dim=0)
        best_map = 0.0

        if self.activate_func == 'default_merge_aux':
            mAP_score_text = mAP(targets, preds_text.cpu().numpy())
            print("Text prompt global mAP score:", mAP_score_text)
            
            if len(self._y_text_local) > 0:
                mAP_score_aux_text = mAP(targets, preds_aux_text.cpu().numpy())
                print("Text prompt local mAP score:", mAP_score_aux_text)
                            
            mAP_score_image = mAP(targets, preds_image.cpu().numpy())
            print("Image prompt global mAP score:", mAP_score_image)
            
            if len(self._y_image_local) > 0:
                mAP_score_aux_image = mAP(targets, preds_aux_image.cpu().numpy())
                print("Image prompt local mAP score:", mAP_score_aux_image)
                
            preds_merge_001 =  preds_image.cpu().numpy() + preds_text.cpu().numpy() + preds_aux_image.cpu().numpy() + preds_aux_text.cpu().numpy()

            mAP_score_tai_merge_001 = mAP(targets, preds_merge_001)
            print("mAP_score_tai_merge_001:", mAP_score_tai_merge_001)

            if self.cfg.TEST.SAVE_PREDS:
                with open(self.cfg.TEST.SAVE_PREDS, 'wb') as f:
                    pickle.dump((targets, preds_merge_tai), f)
        ...
        results = OrderedDict()
        results["mAP"] = mAP_score_tai_merge_001
