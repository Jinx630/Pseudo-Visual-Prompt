    ...
    def forward(self, image=None, captions=None, if_test=False):
        if if_test:    
            def horizontal_flip(image):
                ...
                return [logits_text_global, logits_text_local, logits_image_global]
            
            aug_num, bs, c, h, w = image.shape
            image = image.reshape(-1, c, h, w)
            origin_score = horizontal_flip(image)
            final_score = []
            ...
            return logits_text_global, logits_text_local, logits_image_global, tai_logits, iat_logits
...
class Caption_distill_double(TrainerX):
    ...
    def parse_batch_test(self, batch, use_tta):
        ...
        if use_tta:
            ...
            # image_size =  batch["image_size"][0][0]
            # input_aug_auto = batch['aug_img_auto'].permute(1, 0, 2, 3, 4)
            # input_aug_rotate = batch["aug_img_rotate"].unsqueeze(0)
            # input_aug_gaussianBlur = batch["aug_img_gaussianBlur"].unsqueeze(0)
            # input_aug_colorJitter = batch["aug_img_colorJitter"].unsqueeze(0)
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            text_global, text_local, image_global = self.model_inference(input)
            self.evaluator.process(text_global, image_global, label, text_local)
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            text_global, text_local, image_global, tai, iat = self.model(None, image)
            ce = nn.CrossEntropyLoss(reduction = 'sum')
            ce_label = torch.arange(tai.size(1)).to(tai.device)
            if self.cfg.TRAIN.LOSSFUNC == 'double_ranking':
                loss_text_global = ranking_loss(text_global, label, scale_ = 1.0, margin_ = 1)
                loss_text_local = 0.01 * ranking_loss(text_local, label, scale_ = 1.0, margin_ = 1)
                loss_image_global = ranking_loss(image_global, label, scale_ = 1.0, margin_ = 1)
                # loss_image_local = ranking_loss(image_local, label, scale_ = 1.0, margin_ = 1)
                loss_ce = (ce(tai, ce_label) + ce(iat, ce_label)) / 2
                loss = loss_text_global + loss_text_local + loss_image_global + loss_ce
            else:
                assert False, 'Not implemented.'
