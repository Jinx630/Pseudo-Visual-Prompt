...
class MLClassification(EvaluatorBase):
    ...
    def process(self, image_global, gt, image_local):
        # gt (torch.LongTensor): ground truth [batch]
    ...
    def evaluate(self):
        ...
        if self.activate_func == 'default_merge_aux':
            ...
            if len(self._y_image_local) > 0:
                ...
                r = self.cfg.TRAINER.Caption.GL_merge_rate
                preds_merge_image = preds_image.cpu().numpy() * r + preds_aux_image.cpu().numpy() * (1 - r)
                mAP_score_image_merge = mAP(targets, preds_merge_image)
                print("Image prompt merge mAP score:", mAP_score_image_merge)
        else:
            assert False, 'Not implemented.'

        print("mAP score:", mAP_score_image_merge)

        results = OrderedDict()
        results["mAP"] = mAP_score_image_merge
